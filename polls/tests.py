...
class QuestionDetailViewTests(TestCase):
    ...
    class VoteTest(TestCase):
        def test_vote_count(self):
            ...
            Vote.objects.create(question=question1, choice=choice1, user=User.objects.create_user(username='mymelody'))
        ...
        def test_user_can_vote_one_choice_each_question(self):
            ...
            self.client.post(reverse('polls:vote', args=(question1.id,)),
                             {'choice': choice1.id})
            self.assertEqual(question1.vote_set.get(user=User.objects.create_user(username='mymelody')).choice,
                             choice1)
        ...
        def test_voter_is_authenticated(self):
            ...
            question1 = create_question(question_text='Past Question.', days=-5)
            ans_a = self.client.post(reverse('polls:vote', args=(question1.id,)))
            self.assertEqual(ans_a.status_code, 200)
            self.client.logout()
            ans_b = self.client.post(reverse('polls:vote', args=(question1.id,)))
